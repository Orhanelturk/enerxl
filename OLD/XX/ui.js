window.UI = {
  ...
  init() {
    ...
    this._ensurePanelScrollUnderHeader();
  ...
  _ensurePanelScrollUnderHeader() {
    document.querySelectorAll(".panel").forEach((panel) => {
      if (!panel) return;

      const children = Array.from(panel.children || []);
      const hasBody = children.some(el => el && el.classList && el.classList.contains("panel-body"));
      const head = children.find(el => el && el.classList && el.classList.contains("pa-head")) || null;

      if (hasBody) {
        panel.classList.add("has-body");
        return;
      }
...
(function () {
  const PV = (window.PVDesign = window.PVDesign || {});
  if (PV._wired) return;
  PV._wired = true;
  ...
  function setOrientation(mode) {
    state.orientation = (mode === "landscape") ? "landscape" : "portrait";
    q("pv-orientation-portrait")?.classList.toggle("active", state.orientation === "portrait");
    q("pv-orientation-landscape")?.classList.toggle("active", state.orientation === "landscape");
  }

  function setSelectingEdge(on) {
    state.selectingEdge = !!on;
    const btn = q("pv-align-edge");
    const st = q("pv-edge-status");
    if (btn) btn.classList.toggle("active", state.selectingEdge);
    if (st) st.textContent = state.selectingEdge ? "Edge selection: click an edge on the map…" : "";

    try {
      window.dispatchEvent(new CustomEvent("enerx:pv-align-edge", {
        detail: { active: state.selectingEdge }
      }));
    } catch (_) {}
  }

  PV.getState = function () {
    return JSON.parse(JSON.stringify({
      tiltDeg: q("pv-tilt")?.value || "",
      azimuthDeg: q("pv-azimuth")?.value || "",
      moduleHeightM: q("pv-module-height")?.value || "",
      moduleWidthM: q("pv-module-width")?.value || "",
      orientation: state.orientation,
      modulesPerTableX: q("pv-modules-x")?.value || "",
      modulesPerTableY: q("pv-modules-y")?.value || "",
      tablesPerBlockX: q("pv-tables-x")?.value || "",
      tablesPerBlockY: q("pv-tables-y")?.value || "",
      distModulesX: q("pv-dist-mod-x")?.value || "",
      distModulesY: q("pv-dist-mod-y")?.value || "",
      distTablesX: q("pv-dist-table-x")?.value || "",
      distTablesY: q("pv-dist-table-y")?.value || "",
      distBlocksX: q("pv-dist-block-x")?.value || "",
      distBlocksY: q("pv-dist-block-y")?.value || "",
      selectingEdge: state.selectingEdge
    }));
  };

  PV.init = function () {
    const panel = q("panel-pv-design");
    if (!panel || PV._inited) return;
    PV._inited = true;

    q("pv-orientation-portrait")?.addEventListener("click", () => setOrientation("portrait"));
    q("pv-orientation-landscape")?.addEventListener("click", () => setOrientation("landscape"));
    setOrientation("portrait");

    q("pv-align-edge")?.addEventListener("click", () => setSelectingEdge(!state.selectingEdge));
  };
})();
...
(function () {
  ...
  window.UI.init = function () {
    ...
    if (window.PVDesign && typeof window.PVDesign.init === "function") {
      window.PVDesign.init();
    }
  };
})();
