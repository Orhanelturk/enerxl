...
window.mountDrawing = function mountDrawing(ctx){
  ...
  const btnBuilding=document.getElementById('act-building');
    ...
    // NEW: PV Area by coordinates controls
  const areaCoordsInput = document.getElementById('area-coords-input');
  const areaCoordsError = document.getElementById('area-coords-error');
  const areaCoordsDraw  = document.getElementById('area-coords-draw');
  ...
  Object.entries(shapeBtns).forEach(([k,btn])=>{
    ...
    // === PV Area by Coordinates: click handler ===
  if (areaCoordsDraw && areaCoordsInput && areaCoordsError) {
    areaCoordsDraw.onclick = () => {
      const raw = areaCoordsInput.value.trim();
      areaCoordsError.textContent = '';

      if (!raw) {
        areaCoordsError.textContent = 'Paste coordinates first.';
        return;
      }
      ...
      lines.forEach((line, idx) => {
        ...
        // 1) split by tab / semicolon / space (Excel paste)
        let parts = t.split(/[\t; ]+/).filter(Boolean);

        // 2) if still one token, try "lat,lng"
        if (parts.length === 1) {
        ...
        pathLL.push(new google.maps.LatLng(lat, lng));
      });

      if (pathLL.length < 3) {
        areaCoordsError.textContent = 'Need at least 3 valid points to form an area.';
        return;
      }
      if (bad.length) {
        areaCoordsError.textContent = 'Invalid numbers in rows: ' + bad.join(', ');
        return;
      }

      // Force category to PV area (so styling & logic are consistent)
      category = 'area';
      if (seg) {
        seg.querySelectorAll('button').forEach(b => {
          b.classList.toggle('active', b.dataset.cat === 'area');
        });
        updateDrawingOptions();
      }

      createAreaFromCoordinatePath(pathLL);
      // optional: clear input after success
      // areaCoordsInput.value = '';
    };
  }
  ...
  function styleFor(cat){
    if(cat==='area')       return { fillColor:'#ffffffff',  fillOpacity:.5, strokeColor:'#ffffffff',  strokeWeight:2, clickable:true, editable:false, draggable:false };
    if(cat==='constraint') return { fillColor:'#fbd1d1',  fillOpacity:.5, strokeColor:'#d83b3b',  strokeWeight:2, clickable:true, editable:false, draggable:false };
    // objects style
    return                 { fillColor:'#cfd4dd',  fillOpacity:.5, strokeColor:'#4b5563',  strokeWeight:2, clickable:true, editable:false, draggable:false };
  }
  function selectedStyleOf(cat){
    if(cat==='area')       return { fillColor:'#ffcf66', fillOpacity:1, strokeColor:'#ffb703', strokeWeight:3 };
    if(cat==='constraint') return { fillColor:'#ffd1d1', fillOpacity:1, strokeColor:'#ff7b7b', strokeWeight:3 };
    return                 { fillColor:'#e6eaf1', fillOpacity:1, strokeColor:'#64748b', strokeWeight:3 };
  }
  ...
  // === Create PV Area from a LatLng path (manual coordinates) ===
  function createAreaFromCoordinatePath(pathLL){
    if (!pathLL || pathLL.length < 3) return;
    ...
    poly.__type     = google.maps.drawing.OverlayType.POLYGON;
    poly.__cat      = 'area';
    poly.__selected = false;

    const name = `Area ${++areaCounter}`;
    poly.__name    = name;
    poly.__tilt    = DEFAULT_TILT;
    poly.__azimuth = DEFAULT_AZIMUTH;

    const label = new HtmlLabel(getLabelNW(poly), name);
    label.setMap(map);
    poly.__label = label;

    areas.push({ overlay: poly, type: poly.__type, name, label });

    if (window.__overlayRegistry) {
      window.__overlayRegistry.add(poly);
    }

    bindEvents(poly);
    updateSummary();

    // select + zoom to new area
    clearSelection();
    toggleSelection(poly, true);
    applyModeFlags();

    const b = new google.maps.LatLngBounds();
    pathLL.forEach(p => b.extend(p));
